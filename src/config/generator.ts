...
// - scell_resolve_vat_context: Resolve the applicable VAT rate and category for a single invoice line. POST /api/v1/tenant/buyers/vat-context. Auth: sk_*/pk_*. Provide either buyer_id (UUID of an existing registry entry) or an inline buyer object {country, vatNumber?, isIndividual?}. Provide a line object {category: VatCategory, supplyType?: 'goods'|'services', placeOfSupply?: 'FR'|..., serviceNature?: string}. Returns {resolution: {rate, category, en16931Code, exemptionReason, justification, isAutoResolved, rule}, warnings[]}. KEY LLM RULES: (1) ALWAYS call this tool when the buyer is non-French (country !== 'FR') AND has a vat_number — the rate is very likely 0%. (2) Set supplyType='goods' for cross-border sales of physical goods → resolves to INTRACOM_GOODS (K, art. 262 ter, intra-EU) or EXPORT (G, art. 262 I, non-EU); leave it (services) for INTRACOM services → REVERSE_CHARGE (AE, art. 283-2) or OUT_OF_SCOPE (O, art. 259-1 non-EU). (3) Call before scell_create_invoice when the LLM cannot determine the rate with confidence. (4) Surface justification and exemptionReason to the user before creating the invoice (e.g. 'Livraison intracom de biens → exonération art. 262 ter, code K, taux 0%'). (5) When the user says 'prestation B2B intracom', 'services intracom', 'reverse charge', or the buyer is EU B2B + has a valid VAT number, assume REVERSE_CHARGE (services) or INTRACOM_GOODS (goods) and confirm via this tool. New categories: INTRACOM_GOODS (K), EXPORT (G), FRANCHISE_BASE (293 B), EXEMPT_TRAINING (261-4-4°a).
//
...
// - scell_update_supplier: Partial update of a supplier's contact metadata. PATCH /api/v1/suppliers/{id}. Body: any subset of { email?, phone?, notes?, metadata? } — identity fields (name, country, billing_address, siret, vat_number, legal_id, is_individual) are IMMUTABLE because suppliers are derived from received invoices (source of truth = invoice). Returns updated Supplier. 404 if outside caller scope.
//
// --- Product Catalog (since v2.35.0, tenant key required) ---
// The product catalog is a registry of reusable products / services (label, unit price, default VAT rate, unit…), scoped strictly by (tenant_id, sub_tenant_id) — a tenant NEVER sees another tenant's products, a sub-tenant only sees its own, the parent tenant does not see its sub-tenants' products (same scoping policy as the buyers registry). Products can be referenced by product_id on an invoice / quote LINE to pre-fill it; like buyers, the catalog carries the current state of the article and modifying a product afterwards never alters previously emitted invoices (ISCA compliance). Products can be grouped into product categories.
// - scell_list_products: List products for the calling scope. GET /api/v1/products. Auth: sk_*/pk_* (Sanctum SPA also accepted). Optional query: q (free-text search on name/sku), revenue_category ('goods'|'service'|'accommodation'), product_category_id (UUID filter), is_active (boolean), sub_tenant_id (anti-IDOR: must belong to caller tenant), page, per_page (max 100). Returns PaginatedResult<Product>. Each Product carries id, name, description, sku, revenue_category, revenue_category_label, unit (UN/ECE Rec 20, default 'C62'), unit_price_ht (number), default_tax_rate (number), default_discount_rate (number|null), currency (default 'EUR'), is_active, product_category_id, product_category (nested object|null), metadata, notes.
// - scell_get_product: Retrieve a product by UUID. GET /api/v1/products/{id}. Auth: sk_*/pk_*. Returns Product. 404 if the UUID does not exist in the caller scope (anti-IDOR — does not leak existence to other tenants).
// - scell_create_product: Create a catalog product. POST /api/v1/products. Body: { name (required), unit_price_ht (number, required), product_category_id? (UUID), description?, sku?, revenue_category? ('goods'|'service'|'accommodation'), unit? (UN/ECE Rec 20 code, default 'C62'), default_tax_rate? (number, e.g. 20), default_discount_rate? (number), currency? (ISO 4217, default 'EUR'), is_active? (boolean, default true), sub_tenant_id? (anti-IDOR), metadata?, notes? }. Returns Product (201). 422 if name or unit_price_ht is missing.
// - scell_update_product: Partial update of a product. PATCH /api/v1/products/{id}. Body: any subset of the create body. Does NOT propagate to previously emitted invoices (snapshots are immutable on the ISCA ledger by design). Returns updated Product. 404 if outside caller scope. (A full PUT /api/v1/products/{id} replace is also accepted by the API.)
// - scell_delete_product: Delete a product. DELETE /api/v1/products/{id}. The product becomes unselectable for new invoice/quote lines but historic invoice snapshots remain intact. Returns 204. 404 if outside caller scope.
// - scell_list_product_categories: List product categories for the calling scope. GET /api/v1/product-categories. Auth: sk_*/pk_*. Optional query: q (free-text search on name), sub_tenant_id (anti-IDOR), page, per_page (max 100). Returns PaginatedResult<ProductCategory>. Each ProductCategory carries id, name, color (#RRGGBB|null), description, position (int), products_count (int, when returned), metadata.
// - scell_get_product_category: Retrieve a product category by UUID. GET /api/v1/product-categories/{id}. Auth: sk_*/pk_*. Returns ProductCategory. 404 if outside caller scope (anti-IDOR).
// - scell_create_product_category: Create a product category. POST /api/v1/product-categories. Body: { name (required), color? (#RRGGBB), description?, position? (int), sub_tenant_id? (anti-IDOR), metadata? }. Returns ProductCategory (201). 422 if name is missing.
// - scell_update_product_category: Partial update of a product category. PATCH /api/v1/product-categories/{id}. Body: any subset of the create body. Returns updated ProductCategory. 404 if outside caller scope. (A full PUT /api/v1/product-categories/{id} replace is also accepted by the API.)
// - scell_delete_product_category: Delete a product category. DELETE /api/v1/product-categories/{id}. Returns 204. 404 if outside caller scope.
//
// --- Electronic Invoicing ---
// - scell_create_invoice: Create a new electronic invoice (invoice_number is NOT a parameter — the server generates it automatically. Supports international parties: SIRET required only for French companies (country=FR). For EU companies, provide vat_number. For non-EU, provide legal_id + legal_id_scheme. Optional sub_tenant_id (UUID) attributes the invoice to a sub-tenant of the calling tenant — anti-IDOR 403 if it does not belong to the API key's tenant. The legacy api_keys.company_id binding was removed in the 2026-05-11 refonte. Optional invoice_type ('standard'|'deposit'|'balance') and parent_quote_id (UUID) link the invoice to a source quote. **Since v2.20.0**: parent_quote_id is now also valid for invoice_type='standard' (or omitted, since 'standard' is the default) — useful to track a standalone invoice that originated from a commercial quote without going through the deposit/balance conversion flow. Anti-IDOR 403 if the quote does not belong to the tenant. For 'deposit'/'balance' invoices, prefer the dedicated conversion tools (scell_convert_quote_to_deposit / scell_convert_quote_to_balance) which set parent_quote_id automatically. **VAT AUTO-RESOLUTION (since v2.19.0)**: when lines[].category is provided WITHOUT vatRate, the MCP layer automatically calls scell_resolve_vat_context for each such line before forwarding to the backend, and fills vatRate + metadata.exemption_reason. When both vatRate AND category are provided, the explicit vatRate is respected as-is. This enables the LLM to specify ONLY the business intent (e.g. category='STANDARD' for a domestic service) and let the resolver handle the tax maths based on the actual buyer profile. **VAT CONTROL FIELDS per line (since v2.32.0)**: supplyType ('goods'|'services') DISCRIMINATES the intra-EU/export exemption — goods → INTRACOM_GOODS (K, art. 262 ter I) / EXPORT (G, art. 262 I), services → REVERSE_CHARGE (AE, art. 283-2) / OUT_OF_SCOPE (O, art. 259-1); ALWAYS set supplyType='goods' for a cross-border sale of physical goods, otherwise the server defaults to services (AE). placeOfSupply (ISO-2) forces art. 259-A CGI. vatOverrideReason (max 500) lets you assume a divergent rate. **SERVER-AUTHORITATIVE 409 (since v2.32.0)**: the backend re-resolves the VAT category for every line; if a submitted vatRate is inconsistent with the seller/buyer context (e.g. 20% on an intra-EU B2B sale to a VAT-registered buyer) AND no vatOverrideReason is given, the API replies **409 VAT_CORRECTION_REQUIRED** with { error, message, corrections[] (line_index, provided_rate, suggested_rate, suggested_category, en16931_code, mention, rule), hint } and DOES NOT persist the invoice. The LLM MUST surface the suggested rate/category/mention to the user, then either re-submit with the suggested values or set vatOverrideReason on the offending line. **CATALOG FIELDS per line (since v2.35.0)**: productId (UUID) pre-fills the line from a catalog product (server copies label, unit price and default VAT rate; values set on the line override them) — forwarded as product_id; saveToCatalog (boolean) saves the line as a catalog product via server-side upsert — forwarded as save_to_catalog; productCategoryId (UUID) files the saved product under a category — forwarded as product_category_id. Discover existing products with scell_list_products / create them with scell_create_product.)
...
// - scell_list_invoice_templates: List invoice/quote templates accessible to the caller. GET /api/v1/invoice-templates. Auth: sk_*. Returns Template[] including system defaults (read-only, scope=system) and tenant/sub-tenant overrides. Each Template carries is_enabled (boolean, default true — false = template skipped by the resolution cascade, system template used instead). Optional query: kind (filter by 'invoice' | 'quote' | 'both'), sub_tenant_id (anti-IDOR).
// - scell_create_invoice_template: Create a new template. POST /api/v1/invoice-templates. Body: { name (required), kind ('invoice' | 'quote' | 'both', required), scope ('tenant' | 'sub_tenant', required), sub_tenant_id? (required when scope='sub_tenant', anti-IDOR), primary_color? (#RRGGBB hex), logo_url? (https URL, see scell_upload_branding_logo helper), footer_html?, notes_html?, mentions_html?, is_default? (boolean — set this template as the default for its (scope, sub_tenant_id, kind) tuple, atomic — only one default per tuple), is_enabled? (boolean, default true — false = template skipped by the resolution cascade, system template used instead). Returns Template.
// - scell_get_invoice_template: Retrieve a template by UUID. GET /api/v1/invoice-templates/{id}. 404 if outside caller scope. System templates are always readable. The returned Template carries is_enabled (boolean, default true — false = template skipped by the resolution cascade, system template used instead).
// - scell_update_invoice_template: Partial update of a tenant/sub_tenant template. PUT /api/v1/invoice-templates/{id}. Body: any subset of CreateInvoiceTemplateInput, including is_enabled (boolean — set false to skip this template in the resolution cascade without deleting it, the system template is used instead). System templates cannot be updated (403). Returns updated Template.
...
//
// --- Document Live Preview (since v3.2.0, tenant key required) ---
// - scell_preview_document: Render a live HTML preview of a draft document (invoice, credit note or quote) using the REAL template, branding and legal mentions of the issuing company — nothing is persisted. POST /api/v1/documents/preview. Body: type ('invoice' | 'credit_note' | 'quote', required), document_number?, buyer? {name, siret, vat_number, email, phone, is_individual, address {line1, line2, postal_code, city, country}}, lines? (max 200, [{description, quantity, unit_price, tax_rate}]), issue_date?, due_date? (YYYY-MM-DD), currency? (ISO 4217), notes?, payment_terms? (≤2000 chars each). Auth: sk_*. Returns raw text/html (Cache-Control: no-store). 422 on validation error.
//
...
// - scell_list_fiscal_closings: List closings. GET /api/v1/tenant/fiscal/closings. **(since v2.29.0)** Optional query: closing_type ('daily'|'monthly'|'annual', default daily), sub_tenant_id (anti-IDOR, restrict to one sub-tenant's isolated chain), limit. Each returned closing carries a download_url.
// - scell_download_fiscal_closing: **(since v2.29.0)** Download a closing's CSV (market format) as raw bytes. GET /api/v1/tenant/fiscal/closings/{closing}/download. Auth: sk_*. Works for daily/monthly/annual closings, including a sub-tenant's isolated chain. Tenant-scoped (404 on a closing belonging to another tenant). Use after scell_list_fiscal_closings to fetch the actual CSV of a specific closing.
...
// - scell_create_credit_note: Create a credit note (avoir). POST /api/v1/credit-notes. Auth: sk_*. Body: { invoice_id (required — an avoir ALWAYS targets an existing invoice, it cannot exist on its own), reason (required), type ('total' | 'partial', required), items (required for type='partial', ignored for 'total'), sub_tenant_id? (anti-IDOR) }. **CRITICAL — partial credit notes MUST select lines of the SOURCE invoice**: each items[] entry references an original invoice line via invoice_line_id (mandatory); the unit price and the EXACT per-line VAT rate are INHERITED from that invoice line — the LLM must NOT invent unit_price/tax_rate. An invoice may mix several rates (20%, 5.5%, VAT-exempt 0%) and each line is credited at its own rate (a partial credit on an exempt line stays 0%). items[].quantity is optional (defaults to the line's full remaining quantity). The credit note is created in draft — call scell_send_credit_note to validate (immutable after, ISCA). RECOMMENDED FLOW: call scell_get_remaining_creditable(invoice_id) FIRST to obtain the creditable lines (with invoice_line_id + remaining_quantity + tax_rate), then select among them.
// - scell_send_credit_note: Validate and send credit note
// - scell_delete_credit_note: Delete draft credit note
// - scell_get_remaining_creditable: List the lines of an invoice that can STILL be credited (after previous credit notes). GET /api/v1/invoices/{invoiceId}/remaining-creditable. Auth: sk_*. Returns { data: { invoice_id, invoice_number, items: [{ invoice_line_id, description, original_quantity, credited_quantity, remaining_quantity, unit_price, tax_rate, remaining_amount_ht }], total_remaining, can_be_credited } }. This is the discovery step before a PARTIAL credit note: pick invoice_line_id(s) from data.items[] and pass them to scell_create_credit_note. If can_be_credited is false, the invoice is fully credited (do not attempt a credit note).
//
...
// - scell_get_branding: Get the current email branding for the master tenant OR a specific sub-tenant. GET /api/v1/branding/tenant (no sub_tenant_id) OR GET /api/v1/branding/sub-tenants/{subTenantId} (anti-IDOR — must belong to caller). Auth: sk_*. Returns Branding {brand_logo_url, brand_primary_color, brand_email_footer, brand_email_signature, brand_email_enabled (boolean — false = emails sent with the default channel branding), computed_email_footer (string|null, READ-ONLY — footer computed from the company identity, used at render time when brand_email_footer is empty), is_complete (boolean), missing_fields[] (string array listing which fields are still null when is_complete=false)}. The LLM should check is_complete before suggesting that branding is fully customized.
// - scell_update_branding: Update email branding for the master tenant OR a specific sub-tenant. PUT /api/v1/branding/tenant (no sub_tenant_id) OR PUT /api/v1/branding/sub-tenants/{subTenantId}. Body: any subset of { brand_logo_url (https URL), brand_primary_color (#RRGGBB), brand_email_footer (text/HTML), brand_email_signature (text/HTML), brand_email_enabled (boolean — false = emails sent with the default channel branding) }. All nullable fields accept null to clear and fall back to Scell.io default. Returns updated Branding (with refreshed is_complete + missing_fields + READ-ONLY computed_email_footer).
// - scell_upload_branding_logo: Get a pre-signed S3 PUT URL for uploading the branding logo (PNG/JPEG, max 2 MB, recommended 256x64). POST /api/v1/branding/tenant/logo-upload-url (no sub_tenant_id) OR POST /api/v1/branding/sub-tenants/{subTenantId}/logo-upload-url with body { mime_type: 'image/png' | 'image/jpeg' }. Returns { url (pre-signed S3 PUT), public_url (final URL once uploaded — pass this to scell_update_branding.brand_logo_url), expires_at }. The LLM should instruct the user to PUT the file to url with Content-Type matching mime_type, then call scell_update_branding with brand_logo_url=public_url. ALTERNATIVE (direct multipart upload, no pre-signed URL needed): POST /api/v1/branding/tenant/logo OR POST /api/v1/branding/sub-tenants/{subTenantId}/logo with a multipart field 'logo' (jpeg/png/webp/svg/svgz, max 2 MB) — returns the flat Branding object directly.
// - scell_preview_branding: Render a live preview of how a branded email (invoice delivery, reminder, signature request) will look with the current branding profile, BEFORE sending anything. GET /api/v1/branding/tenant/preview (no sub_tenant_id) OR GET /api/v1/branding/sub-tenants/{subTenantId}/preview (anti-IDOR — must belong to caller). Auth: sk_*. The Accept request header negotiates the format: 'text/html' (default — returns the rendered HTML, ideal to display in an <iframe srcDoc> for a UI live preview) or 'application/pdf' (returns the PDF rendition). Optional NON-PERSISTED query overrides to preview a change before saving it: brand_primary_color (#RRGGBB), brand_email_footer, brand_email_signature, brand_logo_url. Returns the rendered email as a string (HTML) or binary (PDF). Use this after scell_update_branding / scell_upload_branding_logo so the user can VISUALLY confirm the logo, primary color, footer, and signature render correctly before any email is sent. Surfaces incomplete branding visually (Scell.io default fallback is shown when is_complete=false).
//
...
// - scell_disconnect_subtenant_superpdp: **(since v3.1.0)** Disconnect SuperPDP for a sub-tenant. POST /api/v1/tenant/sub-tenants/{id}/superpdp-disconnect. Auth: sk_*. Revokes the SuperPDP tokens server-side and resets onboarding_status to 'pending_superpdp'. Returns { data: SubTenant, recommended_action }. Already-issued invoices stay immutable (ISCA); future B2B invoices fall back to paper transmission until SuperPDP is reconnected. Use scell_reconnect_subtenant_superpdp to disconnect AND re-open the OAuth flow in one call.
// - scell_reconnect_subtenant_superpdp: **(since v3.1.0)** Force a SuperPDP reconnect for a sub-tenant. POST /api/v1/tenant/sub-tenants/{id}/superpdp-reconnect. Auth: sk_*. Disconnects (revoke + reset) THEN generates a fresh prefilled authorize URL in one call. Returns { authorize_url, state, data: SubTenant }. The LLM should present authorize_url to the human user so they re-run the SuperPDP KYB (open in a new tab). Use this after a failed KYB ('superpdp_failed') or a change of company.
// - scell_mint_subtenant_widget_token: **(since v3.1.0)** Mint a signed token for the scell-onboarding web component in mode="superpdp" (passed as its resume-token attribute). POST /api/v1/tenant/sub-tenants/{id}/superpdp-widget-token. Auth: sk_*. Body: { reset? (boolean) }. Returns { resume_token (a signed URL scoped to ONE sub-tenant, 24h TTL), expires_at }. The widget then opens ONLY the SuperPDP step without exposing the sub-tenant id (the HMAC signature prevents IDOR). Pass reset=true to disconnect (revoke + reset) before minting — the 'force reconnect via embedded widget' path; the destructive action stays on this server-side call, never on the public widget.
...
//
// --- Sub-Tenant Micro-Entrepreneur Thresholds + Fiscal Status (since v2.29.0) ---
// - scell_get_subtenant_thresholds: **(since v2.29.0)** Get the French micro-entrepreneur threshold gauges for a sub-tenant. GET /api/v1/tenant/sub-tenants/{id}/thresholds. Auth: sk_*. Returns { data: ThresholdReport, disclaimer }. ThresholdReport = { sub_tenant_id, tenant_id, fiscal_year, generated_at, gauges[], new_alerts[] }. Each gauge = { category ('goods'|'service'|'accommodation'), kind ('vat_franchise_base'|'vat_franchise_majored'|'micro_ceiling'), revenue (cumulative HT EUR for the category), threshold (applicable amount, pro-rated on a first partial year), percent (revenue/threshold*100), level ('warning_80'|'warning_90'|'base_exceeded'|'majored_exceeded'|'micro_ceiling_exceeded'|null), actionable (boolean — true once crossed), projected_crossing_date (ISO date or null — extrapolated at current pace) }. Thresholds are DATED fiscal rules (loi 2025-1044, franchise TVA 37.5k/85k + plafond micro 83.6k/203.1k) resolved server-side for the current year. **The report is purely informational (not tax advice) — ALWAYS surface the disclaimer field to the user.** LLM patterns: warn the user when any gauge.level is set (≥80%), and especially when actionable=true (threshold crossed → propose scell_update_subtenant_fiscal_status to flip vat_status to 'liable').
// - scell_simulate_subtenant_thresholds: **(since v2.30.0)** Pre-issuance threshold simulator. POST /api/v1/tenant/sub-tenants/{id}/thresholds/simulate. Auth: sk_*. Body: { amount (number, net/HT of the hypothetical invoice), category ('goods'|'service'|'accommodation') }. Returns { data: ThresholdReport, simulated: { amount, category }, disclaimer }. The returned gauge level/actionable reflect the POST-invoice projected state — use it to warn the micro-entrepreneur BEFORE issuing whether the invoice would cross a VAT-franchise or micro-regime threshold. Read-only (records no alert, fires no webhook). LLM pattern: when the user asks to issue an invoice for an AE sub-tenant and is near a threshold, simulate first, surface any gauge with actionable=true + the disclaimer, then confirm before scell_create_invoice.
// - scell_update_subtenant_fiscal_status: **(since v2.29.0)** Update a sub-tenant's declared fiscal profile. PATCH /api/v1/tenant/sub-tenants/{id}/fiscal-status. Auth: sk_*. Body: { fiscal_regime? ('micro'|'reel'), vat_status? ('franchise'|'liable'), activity_type? ('goods'|'service'|'accommodation'|'mixed'), activity_start_date? (ISO date or null), vat_number? }. Returns { data: SubTenant, message, disclaimer }. **Setting vat_status='liable' flips Scell.io billing to charge VAT: subsequent invoices carry VAT and DROP the art. 293 B franchise mention.** A vat_number is REQUIRED when becoming liable (in this body or already on the sub-tenant) — otherwise 422. The administrative VAT registration (URSSAF / guichet unique INPI) remains the micro-entrepreneur's responsibility — surface this to the user. Typical flow: scell_get_subtenant_thresholds shows a crossed ceiling → confirm with the user → call this tool with vat_status='liable' + vat_number.
//
...
// - scell_get_quote: Retrieve a quote by ID with all lines, deposit schedule, signature evidence, and sealing proof. For signed quotes the sealing object (since v2.33.0) exposes the cryptographic seal: PAdES PDF signature (pades_signed_at) plus Bitcoin blockchain anchoring of the signed PDF's SHA-256 hash via OpenTimestamps (ots_status pending|confirmed|failed, bitcoin_block_height, ots_proof_base64). Surface this to the user as tamper-evident proof of existence/integrity of an accepted quote. Returns 404 if the quote does not belong to the caller tenant.
...
//
// --- Recurring Invoices / Facturation récurrente (since v2.32.0) ---
// A recurring invoice is an abonnement: a template invoice + a recurrence rule. On each due date Scell.io generates ONE occurrence — a real invoice billed to the buyer. emission_mode='auto_send' submits each occurrence to the PDP (PEPPOL / SuperPDP) AND emails the buyer; emission_mode='draft' (default) creates a DRAFT occurrence for human review (not transmitted, not emailed). The schedule runs until the recurrence end is reached, or it is paused/cancelled. All operations are tenant-scoped (anti-IDOR — pass sub_tenant_id to attribute the schedule to a sub-tenant of the calling tenant, 403 if it does not belong). Already generated occurrences (invoices) are immutable on the ISCA ledger — editing the schedule only affects FUTURE occurrences.
// - scell_list_recurring_invoices: List recurring invoice schedules for the calling scope. GET /api/v1/recurring-invoices. Auth: sk_*. Optional query: status ('active' | 'paused' | 'completed' | 'cancelled'), sub_tenant_id (anti-IDOR — must belong to caller tenant), page, per_page (max 100). Returns PaginatedResult<RecurringInvoice>. Each schedule carries id, title, status, recurrence {interval_unit, interval_count, day_of_month?, day_of_week?}, start_date, end_mode, next_run_date, occurrences_generated, emission_mode.
// - scell_get_recurring_invoice: Retrieve a recurring invoice schedule by UUID. GET /api/v1/recurring-invoices/{id}. Auth: sk_*. Returns RecurringInvoice (full template lines, buyer snapshot, recurrence rule, next_run_date / last_run_date, occurrences_generated). 404 if outside caller scope (anti-IDOR — does not leak existence to other tenants).
// - scell_create_recurring_invoice: Create a recurring invoice schedule. POST /api/v1/recurring-invoices. Auth: sk_*. Body: { title (required), sub_tenant_id? (anti-IDOR), buyer_id? (registry UUID — snapshots buyer onto each occurrence) OR inline buyer_name / buyer_country / buyer_is_individual? / buyer_siret? / buyer_vat_number? / buyer_email? / buyer_address? { line1, postal_code, city, country } / buyer_shipping_address?, currency? (ISO 4217, default EUR), output_format? ('facturx' | 'ubl' | 'cii', default facturx), payment_terms?, lines (required array of { description, quantity, unit_price, vat_rate, unit?, discount?, category?, metadata? }), recurrence (required object { interval_unit ('day' | 'week' | 'month' | 'year'), interval_count? (default 1), day_of_month?, day_of_week? }), start_date (required ISO date), end_mode? ('never' | 'on_date' | 'after_occurrences', default never), end_date? (required if end_mode='on_date'), max_occurrences? (required if end_mode='after_occurrences'), emission_mode? ('draft' | 'auto_send', default draft), notify_before_days?, metadata? }. Returns RecurringInvoice. **KEY LLM RULES: (1) ALWAYS clarify the recurrence end with the user before creating — 'never' (runs indefinitely until paused/cancelled), 'on_date' (stops after end_date), or 'after_occurrences' (stops after N invoices). (2) WARN the user that day_of_month=31 is clamped to the last day of short months (Feb 28/29, Apr 30) — same for 29/30 in February. (3) Each occurrence BILLS THE BUYER — confirm before using emission_mode='auto_send' (auto-submits to the PDP + emails the buyer). emission_mode='draft' creates a draft for review (recommended for the first schedule). (4) buyer_email is required for delivery when emission_mode='auto_send' (or use buyer_id pointing to a registry buyer that already has an email).**
// - scell_update_recurring_invoice: Partial update of a recurring invoice schedule. PUT /api/v1/recurring-invoices/{id}. Auth: sk_*. Body: any subset of the create body (all fields optional). Changing the recurrence rule or start_date affects FUTURE occurrences only — already generated occurrences (invoices) remain immutable (ISCA). Returns the updated RecurringInvoice. 404 if outside caller scope.
// - scell_delete_recurring_invoice: Delete a recurring invoice schedule. DELETE /api/v1/recurring-invoices/{id}. Auth: sk_*. Stops the schedule permanently (no further occurrence is ever generated). Already generated occurrences (invoices) are preserved on the ISCA ledger. Returns 204. 404 if outside caller scope. To temporarily suspend instead of deleting, prefer scell_pause_recurring_invoice.
// - scell_pause_recurring_invoice: Pause an active recurring invoice schedule. POST /api/v1/recurring-invoices/{id}/pause. Auth: sk_*. Sets status='paused' — no occurrence is generated while paused. Resume with scell_activate_recurring_invoice. Returns the updated RecurringInvoice. 422 if the schedule is not in 'active' status.
// - scell_activate_recurring_invoice: Resume a paused recurring invoice schedule (or activate a freshly created one). POST /api/v1/recurring-invoices/{id}/activate. Auth: sk_*. Sets status='active' and recomputes next_run_date. Returns the updated RecurringInvoice. 422 if the schedule is 'completed' or 'cancelled' (terminal states cannot be reactivated).
// - scell_cancel_recurring_invoice: Permanently cancel a recurring invoice schedule. POST /api/v1/recurring-invoices/{id}/cancel. Auth: sk_*. Sets status='cancelled' (terminal — no further occurrence is ever generated, and it cannot be reactivated). Use this to definitively end a subscription. Already generated occurrences remain on the ISCA ledger. Returns the updated RecurringInvoice. Prefer scell_pause_recurring_invoice if the user only wants a temporary suspension.
// - scell_run_recurring_invoice_now: Manually trigger an off-cycle occurrence immediately, without waiting for the next due date. POST /api/v1/recurring-invoices/{id}/run-now. Auth: sk_*. Generates one occurrence right now (respecting emission_mode — draft or auto_send + PDP + email) and advances occurrences_generated; the regular schedule (next_run_date) is unaffected. Returns the generated Invoice. **WARNING: with emission_mode='auto_send' this immediately bills the buyer — confirm with the user first.** 422 if the schedule is 'completed' or 'cancelled'.
//
